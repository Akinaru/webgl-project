...
import * as BloomRailSystemConstants from './BloomRailSystem.constants.js'
...
export default class BloomRailSystem
{
    ...
    rebuildRuntimeData()
    {
        ...
        for(const edge of this.graph.edges)
        {
            ...
            if(length <= BloomRailSystemConstants.EPSILON)
            {
                continue
            }
    ...
    moveAnchorTowards(anchor, targetPosition, deltaSeconds)
    {
        ...
        if(remainingStep <= BloomRailSystemConstants.EPSILON)
        {
            return false
        }
        ...
        while(remainingStep > BloomRailSystemConstants.EPSILON && waypointIndex < route.length)
        {
            const waypoint = route[waypointIndex]
            const distance = anchor.distanceTo(waypoint)

            if(distance <= BloomRailSystemConstants.EPSILON)
            {
                waypointIndex++
                continue
            }
    ...
    projectPointOnSegmentXZ(position, start, end)
    {
        ...
        if(abLenSq <= BloomRailSystemConstants.EPSILON)
        {
            return null
        }
