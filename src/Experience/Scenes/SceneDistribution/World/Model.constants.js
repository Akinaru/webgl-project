export const NON_COLLIDABLE_NAME_TOKENS = [
    'water',
    'cascade',
    'sphere',
    'sphère',
    'screen',
    'background'
]

export const COLLIDABLE_OVERRIDE_NAME_TOKENS = [
    'cube',
    'citerne',
    'citerne_instance'
]

export const BIDIRECTIONAL_COLLISION_NAME_TOKENS = [
    'cube'
]

export const VANNE_NAME_TOKENS = [
    'vanne',
    'axe'
]

export const WALKABLE_GROUND_NAME_TOKENS = [
    'sol',
    'ground',
    'floor',
    'chemin',
    'passerelle',
    'platform'
]

export const PALM_TREE_NAME_TOKENS = [
    'palmier',
    'palm'
]

export const PALM_TRUNK_NAME_TOKENS = [
    'tronc',
    'trunk',
    'stipe'
]

export const TRANSPARENT_EXACT_NAMES = new Set([
    'water',
    'room_end1'
])

export const TRANSPARENT_PREFIXES = [
    'tube-water'
]

export const TRANSPARENT_OPACITY = 0.2
export const TUBE_WATER_NAME_TOKENS = ['tube-water']

export const ROOM_END_WINDOW_COLOR = '#d9f3ff'
export const ROOM_END_WINDOW_OPACITY = 0.55
export const ROOM_END_WINDOW_TRANSMISSION = 0.9
export const ROOM_END_WINDOW_ROUGHNESS = 0.62
export const ROOM_END_WINDOW_METALNESS = 0.0
export const ROOM_END_WINDOW_IOR = 1.45
export const ROOM_END_WINDOW_THICKNESS = 0.14
export const ROOM_END_WINDOW_ENV_INTENSITY = 0.65
export const ROOM_END_WINDOW_ATTENUATION_DISTANCE = 0.65
export const ROOM_END_WINDOW_ATTENUATION_COLOR = '#e8f7ff'
export const ROOM_END_WINDOW_DEPTH_WRITE = true
export const ROOM_END_WINDOW_OVERRIDE_ENABLED = false

export const BACKGROUND_DEFAULT_COLOR = '#ffffff'
export const BACKGROUND_DEFAULT_OPACITY = 1
export const BACKGROUND_DEFAULT_DEPTH_WRITE = false
export const BACKGROUND_DEFAULT_SIDE = 'back'
export const BACKGROUND_SCALE_MULTIPLIER = 1
export const BACKGROUND_OVERRIDE_ENABLED = true
export const BACKGROUND_NAME_TOKENS = ['background', 'polygone']
export const BACKGROUND_TEXTURE_ENABLED = true
export const BACKGROUND_TEXTURE_OFFSET_X = 0
export const BACKGROUND_TEXTURE_OFFSET_Y = 0
export const BACKGROUND_TEXTURE_REPEAT_X = 1
export const BACKGROUND_TEXTURE_REPEAT_Y = 1
export const BACKGROUND_TEXTURE_ROTATION = 0
export const BACKGROUND_TEXTURE_CENTER_X = 0.5
export const BACKGROUND_TEXTURE_CENTER_Y = 0.5
