...
uniform vec3 uCascadeHighlightColor;
...
uniform float uCascadeFoamSoftness;
...
float centerDistance = abs((baseUv.x - 0.5) * 2.0);
float edgeFactor = 1.0 - smoothstep(0.12, 0.92, centerDistance);
vec3 waterColor = mix(uCascadeBaseColor, uCascadeHighlightColor, edgeFactor);
...
vec2 domainWarp = vec2(
    cascadeNoise(domainWarpUvA + vec2(0.0, uCascadeTime * 0.08)),
    cascadeNoise(domainWarpUvB - vec2(0.0, uCascadeTime * 0.06))
) - 0.5;
...
vec2 foamDriftUv = vec2(
    (seededBaseUv.x * max((uCascadeFoamNoiseFrequency * 1.7) + 0.0001, 0.0001)) - (uCascadeTime * 0.23) + (domainWarp.x * 0.55),
    (seededBaseUv.y * max((uCascadeFoamNoiseFrequency * 1.9) + 0.0001, 0.0001)) + (uCascadeTime * 0.31) + (domainWarp.y * 0.75)
);
float foamDrift = cascadeNoise(foamDriftUv);
float foamPulse = (sin((seededBaseUv.y * (20.0 + (uCascadeNoiseSeed.x * 11.0))) - (uCascadeTime * 8.0) + (foamDrift * 6.28318530718)) * 0.5) + 0.5;
float foamField = (foamNoise * 0.45) + (foamDrift * 0.3) + (foamPulse * 0.25);
float foamMask = smoothstep(
    max(0.0, uCascadeFoamThreshold - uCascadeFoamSoftness),
    min(1.5, uCascadeFoamThreshold + uCascadeFoamSoftness),
    foamField
);
foamMask *= uCascadeFoamIntensity;
float foamMaskBinary = step(0.5, foamMask);

vec3 viewDirection = normalize(cameraPosition - vCascadeWorldPosition);
float fresnel = pow(1.0 - max(dot(normalize(vCascadeWorldNormal), viewDirection), 0.0), 2.0);
vec3 finalCascadeColor = mix(waterColor, uCascadeFoamColor, foamMaskBinary);
finalCascadeColor = mix(finalCascadeColor, uCascadeHighlightColor, fresnel * 0.18);
